...
/*static uint8_t FXOS8700CQInitSleep(I2C_HandleTypeDef *I2Cx, int threshold, uint8_t type)
{
	...
	if(check == FXOS8700CQ_WHOAMI_VAL)
	{
		...
		//Configurando o valor 1000 para a interrupção em Z
...
static int16_t refMagX = 0;
static int16_t refMagY = 0;
static int16_t refMagZ = 0;
static int16_t magXMax = ( 0x8000 ), magYMax = ( 0x8000 ), magZMax = ( 0x8000 );
static int16_t magXMin = ( 0x7FFF ), magYMin = ( 0x7FFF ), magZMin = ( 0x7FFF );
...
static uint8_t FXOS8700CQInitFlux(I2C_HandleTypeDef *I2Cx)
{
	...
	if(check == FXOS8700CQ_WHOAMI_VAL)
	{
		...
		// ODR = 50Hz, OSR=32, go to ACTIVE mode using M_CTRL_REG1, M_CTRL_REG2 and CTRL_REG1
		...
		// or reading sensor data (0x01...0x06, 0x33...0x38) depending on the function that generated the interrupt
		FXOS8700CQActive(I2Cx);
...
int FXOS8700CQCalibrate(uint8_t type)
{
	...
	for(int i =0; i < 5; i++)
	{
		HAL_Delay(1);
		FXOS8700CQReadAccelMagnData(&hi2c1);
		calibrate = (int)(FXOS8700CQGetData(GET_MAG_Z));
		/*if((int)(FXOS8700CQGetData(GET_MAG_Z)) > calibrate)
		{
			int auxiliar = (int)(FXOS8700CQGetData(GET_MAG_Z));
			if(auxiliar == 0)
...
void TxAppInit(void){
	...
	*/
	...
	/*int32_t z = FXOS8700CQGetData(GET_MAG_Z) * FXOS8700CQGetData(GET_MAG_Z);
	int32_t y = FXOS8700CQGetData(GET_MAG_Y) * FXOS8700CQGetData(GET_MAG_Y);
	int32_t x = FXOS8700CQGetData(GET_MAG_X) * FXOS8700CQGetData(GET_MAG_X);
...
static void OnBtnTimeoutTimer(void *contextid){
	if(app.stt.btnCnt >= CLICKS_TO_TX && app.stt.btnCnt < (CLICKS_TO_CHANGE_MODE)){
		/*FXOS8700CQInitSleep(&hi2c1, FXOS8700CQCalibrate(INTERRUPT_MAIOR_QUE), INTERRUPT_MAIOR_QUE);
		int32_t z = FXOS8700CQGetData(GET_MAG_Z) * FXOS8700CQGetData(GET_MAG_Z);
		int32_t y = FXOS8700CQGetData(GET_MAG_Y) * FXOS8700CQGetData(GET_MAG_Y);
		int32_t x = FXOS8700CQGetData(GET_MAG_X) * FXOS8700CQGetData(GET_MAG_X);
		lastFlux = squareRoot(z + y + x);*/
...
void tratarInterrupcao(void)
{
	...
	if (Int_Source & 0x02)
	{
		//APP_LOG(TS_OFF, VLEVEL_L, "Interrupt due to Magnetometer Vector Magnitude feature\r\n");
		//APP_LOG(TS_OFF, VLEVEL_L, "Magnetic Jamming detected, resetting hard iron estimate...\r\n");
	...
	{
		//APP_LOG(TS_OFF, VLEVEL_L, "IInterrupt due to data ready\r\n");
		...
		//APP_LOG(TS_OFF, VLEVEL_L, "refMagX = %d refMagY = %d refMagZ = %d\r\n", refMagX, refMagY, refMagZ);
	...
	/*if (Int_Source & 0x04)  // Magnetic threshold event detected?
	{
		...
		int32_t y = FXOS8700CQGetData(GET_MAG_Y) * FXOS8700CQGetData(GET_MAG_Y);
		int32_t x = FXOS8700CQGetData(GET_MAG_X) * FXOS8700CQGetData(GET_MAG_X);
		flux = squareRoot(z + y + x);
		if(flux >(lastFlux + 50))
		{
			APP_LOG(TS_OFF, VLEVEL_L, "METAL\r\n");
